...
GLOBAL_HASHTAGS = [
    ...
    "сейчасмодно",     # «сейчас модно» (трендовый охватный тег)
    "рекомендую",      # «рекомендую»
    "топконтент",      # «топ-контент»
    "видео",           # базовый охватный
    "интересно",       # «интересно»
    "смотридоконца",   # «смотри до конца» (удержание)
    "подписка",        # «подписка»
    "шоу",             # шоу
...
def generate_hashtags(
    ...
    по ключевым словам. Поверх всегда добавляется базовый набор и
    глобальные теги сериала (одинаковые для всех частей/серий).
    """
    smart: List[str] = []
    if llm_model:
    ...
    out: List[str] = []
    for t in (global_tags or GLOBAL_HASHTAGS):
        ct = _clean_tag(t)
